...
пять минут. Если активный запуск остаётся в очереди или работе больше 20 минут,
Worker считает его зависшим, отправляет запрос на отмену и запускает замену. Это
не позволяет одному зависшему GitHub Actions run навсегда заблокировать cron и
